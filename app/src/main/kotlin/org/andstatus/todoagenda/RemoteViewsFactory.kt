    ...
    companion object {
        ...
        fun updateWidget(
            ...
            try {
                ...
                val headerScale = prefs.getString("header_text_size", "1.0")?.toFloatOrNull() ?: 1.0f
                val eventScale = prefs.getString("event_text_size", "1.0")?.toFloatOrNull() ?: 1.0f

                val baseHeaderSp = 16f  // o 18f se vuoi più grande di default
                val baseEventSp = 14f   // tipica dimensione per corpo testo

                val finalHeaderSizeSp = baseHeaderSp * headerScale
                val finalEventSizeSp = baseEventSp * eventScale

                val radiusDp = prefs.getString("corner_radius", "8")?.toIntOrNull() ?: 8

                val bgResId = when (radiusDp) {
                    0 -> R.drawable.widget_background_0
                    4 -> R.drawable.widget_background_4
                    16 -> R.drawable.widget_background_16
                    else -> R.drawable.widget_background_8
                }
                rv.setInt(R.id.widget_parent, "setBackgroundResource", bgResId)


                configureWidgetHeader(settings, rv, finalHeaderSizeSp)
        ...
        private fun configureWidgetHeader(
            ...
            if (settings.widgetHeaderLayout != WidgetHeaderLayout.HIDDEN) {
                ...
                configureCurrentDate(settings, rv, finalHeaderSizeSp)
        ...
        private fun configureCurrentDate(
            ...
            val fullHeader = if (StringUtil.isEmpty(formattedDate)) {
                "SIAK"
            } else {
                "SIAK – $formattedDate"
            }
            rv.setTextViewText(viewId, fullHeader)
            rv.setTextViewTextSize(viewId, TypedValue.COMPLEX_UNIT_SP, headerTextSizeSp)
